...
async function auditInstallation(installation, token) {
  ...
  const installationRepositories = await github.getPaginated(installationToken.token, installation.repositories_url, result => result.repositories);
  const repositories = installationRepositories.filter(repository => repository.owner.id === installation.account.id && repository.fork === false && repository.archived === false);
