...
const workingDirectory = require("../../src/lib/working-directory");
  ...
  describe("Prepare", function() {
    it("should create the empty working directory", async function() {
      ...
      expect(actual).to.equal(
        `test-temp-dir${path.sep}github-tech-report${path.sep}test-owner${
          path.sep
        }test-name`
      );
    ...
    it("should remove the working directory if it already exists", async function() {
      ...
      const expectedDirectory = `test-temp-dir${path.sep}github-tech-report${
        path.sep
      }test-owner${path.sep}test-name`;
      const expectedSubDirectory = `${expectedDirectory}${
        path.sep
      }test-directory`;
      const expectedFile = `${expectedDirectory}${path.sep}test-file`;
      const fsExistsSync = sandbox.stub(fs, "existsSync");
      fsExistsSync.withArgs(expectedDirectory).returns(true);
      fsExistsSync.withArgs(expectedSubDirectory).returns(true);
      ...
      fsLstatSync.returns({ isDirectory: () => false });
      const fsReaddirSync = sandbox.stub(fs, "readdirSync");
      fsReaddirSync.onFirstCall().returns(["test-directory", "test-file"]);
      fsReaddirSync.returns([]);
